...
class Object():
        ...
        for line in lines:
            ...
            if j%2 == 0:
                for i in range(int((len(line)-1))):
                    line_temp = line[i+1].split(',')
                    try:
                        line_temp[0] = int(line_temp[0]) - 0.5
                        line_temp[1] = int(line_temp[1]) - 0.5
                    except:
                        print("Coordinates should be numeric")
                    if line[0] == 'cashdesks':
    ...
    def VisualHuman(self,scale,screen,h,new_step):
        ...
        if h.GetStatus() == 'I':
            pg.draw.circle(screen, (255, 0, 0), (int(h.GetPosition()[0] * scale), int(h.GetPosition()[1] * scale)), int(scale/5))
        else:
            pg.draw.circle(screen, (100, 240, 50), (int(h.GetPosition()[0] * scale), int(h.GetPosition()[1] * scale)), int(scale/5))
        pg.display.update()
